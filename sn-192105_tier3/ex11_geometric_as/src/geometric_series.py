...
def callback(goal):
    ...
    rospy.loginfo('%s: Executing, creating fibonacci sequence of order %i with seeds 1 and 2 ' % (action_server_name, goal.order))
    ...
    for i in range(1, goal.order):
        if action_server.is_preempt_requested():
            ...
            success = False
            break
        feedback.sequence.append(2*feedback.sequence[i - 1])
